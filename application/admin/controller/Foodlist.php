...
// 上传失败获取错误信息
        ...
        $result = Db::name('food')->insertGetId($data1);
        if ($result <= 0) {
        ...
        $res = Db::name('food_detail')->data($data2)->insert();
        $res = Db::name('food_pic')->data($data3)->insert();
        if ($res > 0) {
            return $this->success('添加成功', url('admin/food/index'));
        } else {
            return $this->error('添加失败');
        }
    }
    ...
    {
        var_dump($id);
        $data1 = Db::table('ml_food')->where('id',$id)->select();
        $data2 = Db::table('ml_food_detail')->where('c_gid',$id)->select();
        $data1 = $data1['0'];
        $data2 = $data2['0'];
        $this->assign('data1',$data1);
        $this->assign('data2',$data2);
        return view('food/foodlistdetail');
...
//         die;
        ...
        $result1 =Db::table('ml_food')->where('id',$id)->update($newInfo1);
        $result2 =Db::table('ml_food_detail')->where('c_gid',$id)->update($newInfo2);
//        dump($id);
//        dump($result1);
//        dump($result2);
//        die();
        if($result1 || $result2){
            $this->success('修改成功', 'admin/food/index');
        } else {
            $this->error('修改失败');
        }
    }
